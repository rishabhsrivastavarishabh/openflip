...
export default function Following() {
  ...
  const fetchFollowing = async () => {
    ...
    try {
      ...
      let mutualFollowing = new Set<string>();
      ...
      if (user) {
        ...
        currentUserFollowing = new Set(userFollowing?.map(f => f.following_id) || []);
        ...
        if (user.id === userId) {
          ...
          mutualFollowing = new Set(mutual?.map(f => f.following_id) || []);
        }
      }
      ...
      const enriched: FollowUser[] = (profiles || []).map(p => ({
        ...
        isMutual: mutualFollowing.has(p.id),
      }));
              ...
              <div key={followedUser.id} className="flex items-center gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {followedUser.full_name && (
                    <p className="text-sm text-muted-foreground truncate">{followedUser.full_name}</p>
                  )}
                </div>
